<div class="question-block">
    ...
    @if ($question->hasLink)
        ...
        <a href="{{ $question->link }}" target="_blank" style="text-decoration: underline">
    ...
    @if ($question->type == 'text')
        ...
        <input type="text" name="{{ $question->name }}" id="{{ $question->name }} " {{--  {{ $question->required ? 'required' : '' }} --}}
            {{ $question->autoAnswer ? 'disabled ' : '' }} class="{{ $question->required ? 'required' : '' }}"
    ...
    @elseif ($question->type == 'textarea')
        ...
        <textarea name="{{ $question->name }}" id="{{ $question->name }}" rows="5" cols="50" {{--  {{ $question->required ? 'required' : '' }} --}}
            class="{{ $question->required ? 'required' : '' }}">
            ...
            @for ($j = 1; $j <= count($matrix[$i]); $j++)
                ...
                @elseif ($matrix[$i][$j]->type == 'number')
                    <input type="number" name="{{ $question->name . '__' . $matrix[$i][$j]->name }}" step="0.01"
                        ...
                        class="{{ $question->required ? 'required' : '' }}" {{-- {{ $question->required ? 'required' : '' }} --}}
                        value="{{ array_key_exists($i, $answerMatrix) && array_key_exists($j, $answerMatrix[$i]) ? $answerMatrix[$i][$j] : '' }}"
                ...
                <tr>
                    @for ($j = 1; $j <= count($matrix[$i]); $j++)
                        ...
                        @elseif ($matrix[$i][$j]->type == 'radio')
                            <td>
                                <input type="radio" name="{{ $question->name . '__' . $matrix[$i][1]->name }}"
            ...
            @for ($i = 1; $i <= count($matrix); $i++)
                <tr>
                    @for ($j = 1; $j <= count($matrix[$i]); $j++)
                        ...
                        @elseif ($matrix[$i][$j]->type == 'radio')
                            <td>
                                <input type="radio" name="{{ $question->name }}" id="{{ $question->name }}"
                                    class="{{ $question->required ? 'required' : '' }}"
    ...
    @if ($question->needsEvidence)
        ...
        <input type="file" name="{{ $question->name . '_evidence' }}" id="{{ $question->name . '_evidence' }}"
            class="{{ $question->required && $file->count() == 0 ? 'required' : '' }}" {{-- {{ $question->required ? 'required' : '' }} --}} />
    @endif
</div>
